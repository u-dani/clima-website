      ...
      {geocodingData && weatherData ? (
        <div className='mb-6 flex flex-col gap-6 lg:flex-row lg:gap-8'>
          ...
          <div className='grid auto-rows-fr gap-4 sm:grid-cols-2 md:grid-cols-3 lg:w-3/4'>
            <Card className='flex h-full w-full items-center justify-between gap-6'>
              <div className='flex  flex-col gap-3'>
                ...
                <span className='text-sm'>
                  {weatherData.main.humidity > 50 ? '👍Normal' : '⚠️Alerta'}
                </span>
              </div>
            ...
            <Card className='flex h-full w-full items-center justify-between gap-6'>
              <div className='flex  flex-col gap-3'>
                ...
                <div className='text-4xl font-medium'>
                  {searchParams.unit === 'imperial' ? (
                    <span>
                      {Math.floor(weatherData.main.feels_like * 1.8 + 32)}°
                    </span>
                  ) : (
            ...
            <Card className='flex h-full w-full items-center justify-between gap-6'>
              <div className='flex  flex-col gap-3'>
                <span>Ventos</span>
                <div className='text-4xl font-medium'>
                  <span>{(weatherData.wind.speed / 3.6).toFixed(1)}</span>
                  <span className='align-middle text-2xl'>km/h</span>
                </div>
                <div className='flex items-center text-sm font-medium'>
                  <span>
                    <LiaLocationArrowSolid
                      className={`transform text-lg ${classRotateWindDeg}`}
                    />
                  </span>
                  <span>{windDeg}</span>
                </div>
            ...
            <Card className='flex h-full w-full items-center gap-6'>
              <div className='flex flex-col gap-3'>
